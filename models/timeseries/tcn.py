...
def causal_conv_block(x, filters, kernel_size, dilation, dropout_rate, block_name, training = None):
    ...
    x = CausalDilatedConv1D(filters, kernel_size, dilation, name_prefix = f'{block_name}_conv1')(x, training = training)
    x = layers.Dropout(dropout_rate, name = f'{block_name}_drop1')(x, training=training)

    x = CausalDilatedConv1D(filters, kernel_size, dilation,name_prefix = f'{block_name}_conv2')(x, training = training)
    x = layers.Dropout(dropout_rate, name = f'{block_name}_drop2')(x, training=training)

    if residual.shape[-1] != filters:
        residual = layers.Conv1D(filters, kernel_size = 1, use_bias=False, name = f'{block_name}_proj')(residual)
...
def build_tcn(input_channels, num_classes, num_blocks, num_channels, kernel_size, dropout, model_name):
    ...
    for i in range(num_blocks):
        dilation = 2 ** i
        x = causal_conv_block(x, filters = num_channels, kernel_size = kernel_size, dilation = dilation, dropout_rate = dropout, block_name=f'tcn_block_{i}')

...
def evaluate_keras_model(model, dataset):
    ...
    return {
        'accuracy': round(float(accuracy), 4),'macro_f1': round(float(macro_f1), 4),'per_class_f1': [round(f, 4) for f in per_class_f1],'confusion_matrix': conf_matrix,
    }
...
def compute_accuracy_drop(baseline_metrics, compressed_metrics):
    return {
        'accuracy_drop': round(baseline_metrics['accuracy'] - compressed_metrics['accuracy'], 4),
        'macro_f1_drop': round(baseline_metrics['macro_f1'] - compressed_metrics['macro_f1'], 4),
    }
    ...
    for layer in model.layers:
        if isinstance(layer, (layers.Conv2D, layers.Conv1D)):
            ...
            sensitivity[layer.name] = {
                ...
                'cv': round(float(np.std(importance) / (np.mean(importance) + 1e-8)), 6),
            }
